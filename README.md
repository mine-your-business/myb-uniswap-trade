...
```sh
aws ssm put-parameter \
    --name "/defi/wallets/meta/address" \
    --type "String" \
    --value "0x1234abcd"
```
...
```sh
sam logs -n ExampleFunction --stack-name myb-uniswap-trade --tail
```
...
```sh
aws cloudformation delete-stack --stack-name myb-uniswap-trade
```
